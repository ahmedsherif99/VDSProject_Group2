...
namespace bench_format {
    ...
    struct bench_parser : qi::grammar<Iterator, bench_format::bench_node_type(), const_skipper> {
        ...
        bench_parser() : bench_parser::base_type(start, "bench_grammar") {
            ...
            multiple_op %= lit("(") >> +(lexeme[+(char_("a-zA-Z_0-9.") - ',')] >> lit(","))
            ...
            primary_input = string("INPUT")[bind(&bench_format::bench_node_type::gate_type, _val) = _1]
                    >> rhs_id[bind(&bench_format::bench_node_type::label, _val) = _1] >> +eol;
            primary_output = string("OUTPUT")[bind(&bench_format::bench_node_type::gate_type, _val) = _1]
                    >> rhs_id[bind(&bench_format::bench_node_type::label, _val) = _1] >> +eol;
